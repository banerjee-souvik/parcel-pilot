...
**Reasoning:** I want CI to fail loudly when something real regresses — like the agent no longer verifying identity before reading out shipment details. Judge evals are noisy and cost tokens every run; text snapshots break whenever the model rephrases something harmlessly. Tool calls and DB state _are_ the agent's behavior, so that's the contract I'm pinning.
...
**What broke:** (1) `toolExecutionMs` from the AI SDK's tool-lifecycle event is a float; my `trace_spans.duration_ms` column is `integer` — every tool-call trace insert failed silently in the background (`after()` swallows the error into a log line, not a crash), so tracing looked fine in the terminal and was quietly broken. (2) I never passed `generateMessageId` to `toUIMessageStream`, so assistant messages got persisted with an empty-string id instead of a real one — worked fine for a single turn, would've been a real problem the moment two assistant messages landed in the same chat. (3) `getRescheduleOptions` searches forward day-by-day for 7 valid reschedule dates — for a shipment guardrails will refuse _no matter what date you pick_ (delivered, out for delivery, customs hold), that search never finds 7 valid dates and never terminates. I caught it because I specifically tried to reschedule a delivered shipment to make sure the refusal path worked, and the request just hung.
...
## 11. Day 3: stream resilience, and a redirect I didn't originally plan
...
## 12. Day 4: evals found a real timezone bug, not just their own flakiness
...
**The finding that mattered most:** `getRescheduleOptions` computed valid dates with local-timezone arithmetic (`cursor.getDay()`) but serialized them with `.toISOString().slice(0,10)` — and `applyAction`'s reschedule branch separately built the stored ETA with a _third_, differently-inconsistent method (`` `${date}T00:00:00Z` ``). `toISOString()` is always UTC; local calendar-day arithmetic is not. Any server timezone ahead of UTC can have the two disagree about which day a date string represents, especially close to local midnight. This was a genuinely latent product bug, not an eval artifact — it just happened to surface _through_ an eval, because I ran the suite locally in IST just after midnight and a date my code had validated as Monday got serialized as, and then correctly rejected as, Sunday. Confirmed by direct computation, not guessing: `new Date()` at that moment really did roundtrip through UTC into the wrong calendar day. Fixed by adding `guardrails.parseDateOnly`/`formatDateOnly` (construct/read calendar dates via local getters only, never through a UTC instant) and routing every reschedule-date touchpoint in `services.ts` through them, plus my own eval helpers, which had copied the same anti-pattern.
...
**One more of the same lesson, caught on the final full-suite run:** `reschedule-invalid-date` asserted the relayed refusal text contained the word "sunday" — but the system prompt explicitly tells the model it can paraphrase refusals, and it did: "that day isn't served on this route," true to the guardrail's meaning, without naming the day. The refusal _code_ assertion (`INVALID_DATE`) already passed — only the prose-adjacent check was too literal. Fixed by checking for "route" instead, a word lifted from the actual guardrail message in `guardrails.ts` that a faithful paraphrase reliably keeps. Same category as the other two flakiness causes above: not a product bug, a test that assumed more verbatim-ness than the product actually promises.

---

## 13. Day 5 so far: the Playwright smoke test found a real production caching bug
...
**What actually broke, three different ways, in order:** (1) A test-only bug — `getByText(chatId)` in strict mode failed because the chat had _multiple_ trace rows, one per turn, exactly as designed (decisions.md #12's traceId-not-chatId choice). Fixed the assertion, not the app. (2) A test-design mistake I introduced myself — an 8-second "did the confirm card appear yet" pre-check followed by a nudge message, added to absorb model non-determinism, actually _caused_ a confusing double-interaction (a second, redundant tool call) when it fired on a response that was just running a little slow. Removed it; `sendAndWaitForReply` already waits out the model's full turn before returning, so a second race on top of that was pure risk with no benefit. (3) The real one: after (1) and (2) were fixed, the `/traces` list _reproducibly_ failed to show a chat's trace rows — but querying Postgres directly during the same failing run proved the rows existed. The bug was Next.js's Full Route Cache: neither `/traces` (via its layout) nor `/` had a `dynamic = "force-dynamic"` export, so `next build`/`next start` was free to statically cache their DB-backed server-component output and serve a stale snapshot instead of querying on every request. Confirmed by killing the running server and forcing a clean rebuild after adding the export — the exact same test then passed twice in a row.
...
## 14. Day 5 polish: closing a real a11y gap, not just a cosmetic one
...
## 15. A real bug found by manual testing, not by any automated suite: `ensureChat`'s race
...
## 16. Chat creation is lazy — a page _view_ should never write a row
...
**Reasoning — why these two asks aren't actually the same kind of guardrail:** "Don't answer off-topic questions" and "don't answer about a different shipment" sound like the same category of request, but they aren't, and I said so before implementing either. A different-shipment request is a _tool call_ with a `trackingNumber` argument — there's a concrete chokepoint in code where it can be intercepted and refused before it touches real data, the same shape as every other guardrail in this app. An off-topic question is just the model replying in prose; there is no tool call to intercept, no chokepoint to guard, nothing for `guardrails.ts` to sit in front of. So the shipment-lock is real, mechanical, code-enforced, and provably works even if the model tries to disobey it. The topic guardrail is a system-prompt instruction (rule 7) and nothing more — it's honest to call it a strong nudge, not a guarantee, and I'd rather say that plainly than let "we added more guardrails" imply a uniformity of enforcement that doesn't exist.

**What the eval caught, twice, in the same scenario:** Building `shipment-session-scope.eval.ts` surfaced the identical "brittle assertion vs. legitimate paraphrase" lesson from decisions.md #12 — twice, back to back, on one new scenario. First: I asserted a tool-call-level refusal _had_ to happen, but a real run showed the model self-enforcing purely from conversation context — refusing the second shipment in prose without ever attempting the tool call at all. That's better behavior than what I'd specified, not a bug; fixed by asserting the fact that actually matters (`chats.scopedTrackingNumber` never moves off the first shipment, checked directly against the DB) instead of assuming one specific enforcement path. Second, immediately after: my new "must say new chat" text check failed on a real run that said "start a new **conversation**" instead — same lesson as the "sunday"/"route" fix, same fix shape (match a set of acceptable synonyms, not one literal string). Stable across three consecutive real-model runs after both fixes.
...
## 19. Dropping Gemini — Groq-only from here on
...
## 20. Same Full Route Cache bug, third time: `/chat` was silently funneling everyone into one chat

**Decision:** Added `export const dynamic = "force-dynamic"` to `chat/page.tsx`.

**What broke:** Reported from production — clicking "New conversation" kept reopening the same chat instead of starting a fresh one. `chat/page.tsx` is a Server Component with no data fetching, just `redirect(`/chat/c_${nanoid(12)}`)` — nothing about it screams "needs a live server," which is exactly how it slipped past both prior rounds of this same bug (#13, and the fix on `/` and `/traces/layout.tsx` that followed it). `nanoid()` doesn't touch any request-time API (no `cookies()`, `headers()`, `searchParams`), so Next's static optimizer had nothing telling it this page couldn't be prerendered — and it doesn't need one, since a component that only calls `redirect()` typechecks and runs identically whether it's evaluated once at build time or fresh per request. `next build` ran it once, baked that single `nanoid()` result into a static redirect `Location`, and every "New conversation" click in production hit the same frozen chat id until the next deploy. Confirmed directly: `yarn build` output showed `○ /chat` (static); after the fix, `ƒ /chat` (dynamic), and three separate `curl` requests against `yarn start` returned three different chat ids.

**Reasoning:** This is the third time this exact mechanism has bitten this app (`/` and `/traces` in decision #13), and the pattern is the same each time: a component with no obvious DB/data dependency looks safe to prerender, and Next's static optimizer agrees, silently, with no build warning that anything unusual happened. The lesson isn't "remember to check this route too" — it's that any Server Component calling `redirect()` with a request-independent side effect (a fresh id, a random value, `Date.now()`) needs `force-dynamic` on principle, whether or not it happens to read from the DB. Doing a repo-wide grep for `redirect(` and checking each one's `dynamic` export would have caught this before it shipped instead of after.

**Deliberately cut:** Nothing — this is the kind of bug that's invisible until an evaluator (or the person building it) refreshes twice in a row.
