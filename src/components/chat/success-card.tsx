...
const RECEIPT_LABEL: Record<Receipt["kind"], string> = {
  reschedule: "Delivery rescheduled",
  change_address: "Address updated",
  update_instructions: "Instructions saved",
  file_claim: "Claim filed",
};
...
export function SuccessCard({ receipt }: { receipt: Receipt }) {
  ...
  return (
    <div
      role="group"
      aria-label={label}
      className="flex w-full flex-col gap-2 rounded-2xl border border-success-border bg-success-soft px-3.5 py-3"
    >
...
export function CancelledCard() {
  return (
    <div role="group" aria-label="Cancelled" className="flex w-full items-center gap-2 rounded-2xl border border-border bg-bg-subtle px-3.5 py-2.5">
